...
class GetPromo extends AbsensiProvider {
  Future<PromoModel> getPromo() async {
    ...
    for (int retry = 0; retry < maxRetry; retry++) {
      final response = await get("$url" "get-status", headers: {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            "Kesalahan sisi client saat mengambil status absensi. Kode status: ${response.statusCode}");
        // Anda dapat menampilkan pesan kesalahan khusus untuk kesalahan sisi client
        throw Exception("Kesalahan sisi client saat mengambil status absensi");
      } else if (response.statusCode! >= 500 && response.statusCode! < 600) {
        // Kesalahan sisi server (5xx)
        print(
            "Kesalahan sisi server saat mengambil status absensi. Kode status: ${response.statusCode}");
        // Anda dapat menampilkan pesan kesalahan khusus untuk kesalahan sisi server
        throw Exception("Kesalahan sisi server saat mengambil status absensi");
      } else {
        // Kode status lain yang tidak diharapkan
        print(
            "Kesalahan tidak diketahui saat mengambil status absensi. Kode status: ${response.statusCode}");
    ...
    throw Exception(
        "Gagal mengambil status absensi setelah $maxRetry kali percobaan");
  }
}
